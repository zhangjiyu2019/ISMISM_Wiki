            ...
            <section className="rounded-xl border border-white/10 bg-zinc-900/70 p-4">
              ...
              >
                <div className="cube" style={{ transform: `rotateX(${rotation.x}deg) rotateY(${rotation.y}deg)` }}>
                  ...
                  <div className="face right face-ontology">
                    <div className="face-grid">
                      {faceFillers.map((item) => (
                        <span key={`right-${item}`} className="sticker" />
                      ))}
                    </div>
                  </div>
                  <div className="face top face-epistemology">
                    <div className="face-grid">
                      {faceFillers.map((item) => (
                        <span key={`top-${item}`} className="sticker" />
                      ))}
                    </div>
                  </div>
                  <div className="face back face-back">
                    <div className="face-grid">
                      {faceFillers.map((item) => (
                        <span key={`back-${item}`} className="sticker" />
                      ))}
                    </div>
                  </div>
                  <div className="face left face-left">
                    <div className="face-grid">
                      {faceFillers.map((item) => (
                        <span key={`left-${item}`} className="sticker" />
                      ))}
                    </div>
                  </div>
                  <div className="face bottom face-bottom">
                    <div className="face-grid">
                      {faceFillers.map((item) => (
                        <span key={`bottom-${item}`} className="sticker" />
                      ))}
                    </div>
                  </div>
                </div>
